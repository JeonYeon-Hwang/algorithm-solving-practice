...
def merge(arr, left, mid, right):
    ...
    while i <= mid:
        temp_left_arr.append(arr[i])
        i += 1
    while j <= right:
        temp_right_arr.append(arr[j])
        j += 1
    pass
    ...
    ln = len(temp_left_arr)
    rn = len(temp_right_arr)

    # TODO: left_arr와 right_arr를 비교하며 작은 값을 arr에 복사
    while arr_idx <= right and l < ln and r < rn:
        if temp_left_arr[l] > temp_right_arr[r]:
            arr[arr_idx] = temp_right_arr[r]
            r += 1
        else:
            arr[arr_idx] = temp_left_arr[l]
            l += 1
        arr_idx += 1
    pass
    # TODO: 남은 원소들을 복사
